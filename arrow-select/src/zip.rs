...
use arrow_array::*;
use arrow_buffer::{BooleanBuffer, Buffer, MutableBuffer, NullBuffer, OffsetBuffer, OffsetBufferBuilder, ScalarBuffer, ToByteSlice};
...
impl<T: ByteViewType> ByteViewScalarImpl<T> {
    ...
    fn get_scalar_buffers_and_nulls_for_all_values_null(len: usize)
        -> (ScalarBuffer<u128>, Vec<Buffer>, Option<NullBuffer>) {
        let mut mutable = MutableBuffer::with_capacity(0);
        mutable.repeat_slice_n_times((0 as u128).to_byte_slice(), len);
    ...
    fn get_scalar_buffers_and_nulls_non_nullable(
        ...
        // if truthy has non-inlined values, include the buffers
        if truthy.total_buffer_bytes_used() > 0 {
            buffers.extend(truthy.data_buffers().to_vec());
        }

        // if falsy has non-inlined values, include the buffers and recalculate the view
        ...
        let total_number_of_bytes =
            true_count * view_truthy.len() + (predicate.len() - true_count) * view_falsy.to_byte_slice().len();
        ...
        (bytes.into(), buffers, Some(NullBuffer::new_valid(predicate.len())))
    }
    ...
    fn create_output_on_non_nulls(
        ...
        match true_count {
            ...
            _ => {
                Self::get_scalar_buffers_and_nulls_non_nullable(predicate, truthy, falsy)
            }
        }
    }
}
...
impl<T: ByteViewType> ZipImpl for ByteViewScalarImpl<T> {
    fn create_output(&self, predicate: &BooleanArray) -> Result<ArrayRef, ArrowError> {
        ...
        let (views, buffers, nulls) = match (self.truthy.as_ref(), self.falsy.as_ref()) {
            ...
            (None, None) => {
                Self::get_scalar_buffers_and_nulls_for_all_values_null(result_len)
            }
        };

        let result = unsafe {
            StringViewArray::new_unchecked(views.into(), buffers, nulls)
        };
        Ok(Arc::new(result))
    }
}
